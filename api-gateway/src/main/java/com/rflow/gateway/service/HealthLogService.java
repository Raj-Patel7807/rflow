...
public class HealthLogService {
    ...
    public PagedHealthLogResponse getHealthLogs(Long tenantId, Long serviceId, int page, int size) {
        ...
        if(serviceIds.isEmpty()) {
            return PagedHealthLogResponse.builder().logs(List.of()).page(page).size(size).totalElements(0).totalPages(0).build();
        }
        ...
        return PagedHealthLogResponse.builder().logs(logs.stream().map(log -> map(log, serviceNames)).toList()).page(page).size(size).totalElements(total).totalPages(size == 0 ? 0 : (int) Math.ceil((double) total / size)).build();
    }
    ...
    private HealthLogResponse map(ServiceHealthLog log, Map<Long, String> serviceNames) {
        return HealthLogResponse.builder().id(log.getId()).serviceId(log.getServiceId()).serviceName(serviceNames.get(log.getServiceId())).healthStatus(log.getHealthStatus()).responseTimeMs(log.getResponseTimeMs()).checkedAt(log.getCheckedAt()).build();
    }

    public HealthLogResponse checkService(Long tenantId, Long serviceId) {
        BackendService service = serviceRepository.findById(serviceId).orElseThrow(() -> new RuntimeException("Service Not Found"));
